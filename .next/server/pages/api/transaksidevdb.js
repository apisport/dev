...
async function getTransaksi(req, res) {
    ...
    try {
        ...
        let transaksi = await db.collection('transaksi').find({}).sort({
            idfavorit: -1
        }).toArray();
        return res.json({
...
async function updateTransaksi(req, res) {
    const { tim , noRekening , opsiBayar , buktiBayar , objectId , hargaDP  } = req.body;
...
var __webpack_exports__ = __webpack_require__.X(0, [6198], () => (__webpack_exec__(8841)));
